...
public class SimpleQueries {
    ...
    public static int binarySearch(int[] nums, int i,int start, int end){
        if(start>end) return -1;
        int mid = (start+end)/2;
        if(nums[mid]==i||(nums[mid]<i&&(mid+1>=nums.length||nums[mid+1]>i))) return mid;
        else if(nums[mid]<i&&nums[mid+1]<i) return binarySearch(nums,i,mid+1,end);
        else if(mid-1>=0&&nums[mid]>i) return binarySearch(nums,i,start,mid-1);
